...
#include "PokemonSwSh_EggAutonomousKeep.h"
...
EggAutonomousKeep_Descriptor::EggAutonomousKeep_Descriptor()
    : SingleSwitchProgramDescriptor(
        "PokemonSwSh:EggAutonomousKeep",
        STRING_POKEMON + " SwSh", "Egg Autonomous (Keep)",
        "Programs/PokemonSwSh/EggAutonomousKeep.html",
...
class EggAutonomousKeep_Descriptor::Stats : public StatsTracker{
public:
    Stats()
...
std::unique_ptr<StatsTracker> EggAutonomousKeep_Descriptor::make_stats() const{
    return std::unique_ptr<StatsTracker>(new Stats());
}


EggAutonomousKeep::EggAutonomousKeep()
...
void EggAutonomousKeep::program(SingleSwitchProgramEnvironment& env, ProControllerContext& context){
    auto& stats = env.current_stats<EggAutonomousKeep_Descriptor::Stats>();
    env.update_stats();
...
bool EggAutonomousKeep::run_batch(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
    EggAutonomousKeep_Descriptor::Stats& stats
){
    env.update_stats();
...
bool EggAutonomousKeep::run_bike_loop(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context
){
...
void EggAutonomousKeep::exceed_bike_loop_limit(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
...
size_t EggAutonomousKeep::hatch_routine(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
    EggAutonomousKeep_Descriptor::Stats& stats,
...
bool EggAutonomousKeep::call_flying_taxi(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
...
void EggAutonomousKeep::wait_for_egg_hatched(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
    EggAutonomousKeep_Descriptor::Stats& stats,
...
EggFetchResult EggAutonomousKeep::talk_to_lady_to_fetch_egg(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
    EggAutonomousKeep_Descriptor::Stats& stats
){
...
// - Call flying taxi to reset player location if needed
// Return true if the program should stop
bool EggAutonomousKeep::process_hatched_pokemon(
    SingleSwitchProgramEnvironment& env,
    ProControllerContext& context,
    EggAutonomousKeep_Descriptor::Stats& stats,
    bool need_taxi
...
void EggAutonomousKeep::check_box(VideoStream& stream, ProControllerContext& context){
    context.wait_for_all_requests();
    context.wait_for(500ms);
...
void EggAutonomousKeep::check_non_egg_lead(VideoStream& stream, const ImageViewRGB32& screen){
...
size_t EggAutonomousKeep::count_eggs_in_party(VideoStream& stream, const ImageViewRGB32& screen){
    size_t num_eggs = 0;
...
size_t EggAutonomousKeep::count_empty_slots_in_party(VideoStream& stream, const ImageViewRGB32& screen){
    size_t num_empty = 0;
    for (uint8_t row = 0; row < 6; row++){
